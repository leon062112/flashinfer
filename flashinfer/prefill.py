...
class BatchPrefillWithPagedKVCacheWrapper:
    ...
    def workspace_size(
        ...
        # Pass mask_mode and dllm_block_size for block-extend plan optimization.
        if backend in ("fa2", "fa3"):
            args.append(MaskMode.BLOCK_EXTEND.value if self._block_extend else 0)
            args.append(self._block_size or 0)
        args.append(None)  # q_offsets placeholder
    ...
    def plan(
        ...
        if self._cached_module is not None:
            ...
            if self._backend in ("fa2", "fa3"):
                args.append(MaskMode.BLOCK_EXTEND.value if self._block_extend else 0)
                args.append(self._block_size or 0)
            # Pass q_offsets for block-extend scheduling: the scheduler needs
            # the global Q start position to correctly compute effective KV
            # lengths for incremental (step-by-step) prefill.
            if self._block_extend and q_offsets is not None:
                args.append(q_offsets.to("cpu").to(torch.int64).contiguous())
            else:
                args.append(None)
