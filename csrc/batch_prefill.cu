...
Array<int64_t> BatchPrefillWithKVCachePlan(
    ...
    bool disable_split_kv, int64_t num_colocated_ctas = 0, int64_t mask_mode = 0,
    int64_t dllm_block_size = 0) {
  ...
  cudaError_t status = PrefillPlan<IdType>(
      ...
      /*sizeof_dtype_o=*/2, window_left, fixed_split_size, disable_split_kv,
      mask_mode, dllm_block_size, num_colocated_ctas, stream);
