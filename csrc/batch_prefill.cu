...
Array<int64_t> BatchPrefillWithKVCachePlan(
    ...
    int64_t mask_mode = 0, int64_t dllm_block_size = 0,
    Optional<TensorView> q_offsets = Optional<TensorView>()) {
  ...
  cudaError_t status = PrefillPlan<IdType>(
      ...
      /*sizeof_dtype_o=*/2, window_left, fixed_split_size, disable_split_kv, mask_mode,
      dllm_block_size, num_colocated_ctas, uniform_q_len,
      q_offsets.has_value() ? static_cast<IdType*>(q_offsets.value().data_ptr()) : nullptr, stream);
...
Array<int64_t> BatchPrefillWithKVCacheWorkspaceSize(
    ...
    int64_t num_colocated_ctas = 0, int64_t uniform_q_len = 0, int64_t mask_mode = 0,
    int64_t dllm_block_size = 0, Optional<TensorView> q_offsets = Optional<TensorView>()) {
  (void)kv_len_arr;
  (void)causal;
  ...
  cudaError_t status = PrefillPlanWorkspaceSize<IdType>(
      ...
      mask_mode, dllm_block_size, num_colocated_ctas, uniform_q_len,
      q_offsets.has_value() ? static_cast<IdType*>(q_offsets.value().data_ptr()) : nullptr, stream);
